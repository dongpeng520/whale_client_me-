/**
 * Created by peter.dong on 16/12/17.
 */
pictureModule.controller("shareController",["$scope","$window","$http","$interval","$location", function($scope,$window,$http,$interval,$location){
    ...
    if(window.location.href.indexOf("?key=") !== -1){
        if(window.location.href.indexOf("&u=") !== -1){
            var key;
            key=window.location.href.substring(window.location.href.indexOf("?key=")+5,window.location.href.indexOf("&u="));
        }else{
            var key=window.location.href.split("?key=")[1];
        }
        $http.get(pictureAir.host + "g/user/getShareInfo",{
            params: {
            ...
            if(data.status === 200){
                var src;
                src=pictureAir.http+data.result.info[0].url;
