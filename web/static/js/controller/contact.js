/**
 * Created by shdr-gs07 on 16/11/25.
 */
pictureModule.controller("ContactController",["$scope","$location","$window","$http",  function($scope,$location,$window,$http){
    ...
    login_contact()
    $("#con_name").on("keyup", function () {
        var filtered = $(this).val().replace(/[!;'@#%&$<>=+\-\*\(\)\^\\{}]*/, "");
        $(this).val(filtered)
    })
    $("#ppp").on("keyup", function () {
        var filtered = $(this).val().replace(/[!;'@#%&$<>=+\-\*\(\)\^\\{}]*[^\u0000-\u00FF]*/, "");
        $(this).val(filtered)
    })

    $("#feedback").on("keyup", function () {
        var filterdata = $(this).val().replace(/[<>]*/, "")
        $(this).val(filterdata)
    })
    $.extend({
        pwlist: function (ele) {
    ...
    $(".submit").on("click", function () {
        if ($("#con_name").val() == "" || /^\s+ $/.test($("#con_name").val())) {
            if (false) {
                myalert($p.tl("姓名不可为空!", "The name can not be empty!"))
            } else {
                $(".error").text($scope.languagePack.error_er113).show();
            }
            return false;
        }
        if ($("#con_email").val() == "" || /^\s+ $/.test($("#con_email").val())) {
            if (false) {
                myalert($p.tl("邮箱不可为空!", "The email can not be empty!"))
            } else {
                $(".error").text($scope.languagePack.error_er114).show();
            }
            return false;
        }
        if (!pictureAir.email_pat.test($("#con_email").val())) {
            if (false) {
                myalert($p.tl("邮箱格式不正确!", "The email is not valid!"))
            } else {
                $(".error").text($scope.languagePack.error_er115).show();
            }
            return false;
        }
        if ($("#attraction").val() == "" || /^\s+ $/.test($("#attraction").val())) {
            if (false) {
                myalert($p.tl("园区不可为空!", "The project can not be empty!"))
            } else {
                $(".error").text($scope.languagePack.error_er116).show();
            }
            return false;
        }
        if ($("#feedback").val() == "" || /^\s+ $/.test($("#feedback").val())) {
            if (false) {
                myalert($p.tl("描述信息不可为空!", "The browser can not be empty!"))
            } else {
                $(".error").text($scope.languagePack.error_er117).show();
            }
            return false;
        }
        var data = {
            params: {
                access_token: pictureAir.store("access_token"),
                name: $("#con_name").val(),
                EmailAddress: $("#con_email").val(),
                parkName: $("#attraction").val(),
                feedback: $("#feedback").val()
            }
        }
        if ($("#InparkTime").val() !== "" && !/^\s+ $/.test($("#InparkTime").val())) {
            data.params.visitDate =$("#InparkTime").val()+" 00:00:00";
        }
        if ($("#ppp").val() !== "" && !/^\s+ $/.test($("#ppp").val())) {
            data.params.pictureAirCode = $("#ppp").val();
        }
        if ($("#order").val() !== "" && !/^\s+ $/.test($("#order").val())) {
            data.params.orderID = $("#order").val();
        }
        if ($("#myos").val() !== "") {
            data.params.operatingSystem = $("#myos").val()
        }
        $http.get(pictureAir.host + "g/user/contactUs", data).success(function (data) {
            if (data.status === 200) {
                alert($scope.languagePack.error_er118)
            } else {
                alert($scope.languagePack.error_er119)
            }
        });
